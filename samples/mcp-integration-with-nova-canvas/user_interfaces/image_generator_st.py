...
def generate_image(
    ...
    try:
        ...
        response = requests.post(
            f'{API_URL}/generate', json=payload, timeout=120  # Longer timeout for image generation
        )
        response.raise_for_status()  # Raise an exception for HTTP errors
        return response.json()
    except requests.exceptions.RequestException as e:
        st.error(f'API Error: {str(e)}')
        return {'status': 'error', 'message': f'Error communicating with the API: {str(e)}', 'image_paths': []}
...
with st.sidebar:
    ...
    cfg_scale = st.slider('CFG Scale', min_value=1.1, max_value=10.0, value=6.5, step=0.1,
                         help='How strongly the image adheres to the prompt')
    
    use_seed = st.checkbox('Use specific seed', value=False)
    if use_seed:
        seed = st.number_input('Seed', min_value=0, max_value=858993459, value=random.randint(0, 858993459))
    else:
        seed = None
...
if not negative_prompt:
    st.info('Tip: Consider adding "people, anatomy, hands, low quality, low resolution, low detail" to your negative prompt for better results.')
...
use_improved_prompt = bool(st.checkbox('Use Improved Prompt', value=True, help='Use the improved prompt for image generation'))
