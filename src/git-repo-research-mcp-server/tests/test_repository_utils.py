...
def test_load_metadata_invalid_file():
    ...
    with patch('os.path.exists') as mock_exists, \
        ...
        assert 'Error loading metadata from /path/to/metadata.json' in mock_logger_error.call_args[0][0]


def test_list_indexed_repositories_empty_dir():
    """Test listing indexed repositories when the directory is empty."""
    with patch('awslabs.git_repo_research_mcp_server.utils.get_default_index_dir') as mock_get_default_index_dir, \
...
def test_delete_indexed_repository_absolute_path():
    ...
    with patch('awslabs.git_repo_research_mcp_server.utils.get_default_index_dir') as mock_get_default_index_dir, \
        ...
        mock_get_default_index_dir.return_value = '/home/user/.git_repo_research'
        mock_exists.return_value = True
        mock_isabs.return_value = True  # It's an absolute path
        mock_isdir.return_value = True
        mock_isfile.return_value = False
        mock_splitext.return_value = ('/absolute/path/to/repo', '')
        mock_access.return_value = True
        ...
        # Call the function with an absolute path
        result = delete_indexed_repository('/absolute/path/to/repo')
